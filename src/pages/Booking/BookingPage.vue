    ...
    <template v-if="BookingStore.hasData">
      <template v-if="BookingStore.getSteps == 1">
        <q-select
          ...
          @update:model-value="
            BookingStore.getTimeslot(SettingStore.merchant_uuid)
          "
          :label="$t('Guest')"
          dense
          color="primary"
        ...
        <template v-if="SettingStore.settings_data.booking_reservation_terms">
          <div class="text-weight-bold font15 q-mt-md">
            {{ $t("Restaurant Terms & Conditions") }}
          </div>
          <div
            class="text-grey"
            v-html="SettingStore.settings_data.booking_reservation_terms"
          ></div>
        </template>
...
export default {
  ...
  mounted() {
    ...
    if (auth.authenticated()) {
      ...
      this.BookingStore.first_name = userinfo.first_name;
      this.BookingStore.last_name = userinfo.last_name;
      this.BookingStore.email_address = userinfo.email_address;
      this.BookingStore.mobile_number = userinfo.contact_number;
    }
  ...
  methods: {
    ...
    SetBooking() {
      this.BookingStore.SetBooking(this.SettingStore.merchant_uuid);
    },
    recaptchaExpired() {
